...
@click.command()
@click.option('--path', prompt='Full path to .csv file', 
              type=click.Path(exists=True))
@click.option('--wv_range', prompt='Range of wavelengths (i.e. max - min)')
@click.option('--min_conc', prompt='Minimum concentration, in mg/mL')
@click.option('--max_conc', prompt='Maximum concentration, in mg/mL')
@click.option('--step', prompt='Total number of samples in csv')
@click.option('--vb1', prompt='Lambda max/first vibronic band')
@click.option('--vb3', prompt='Third vibronic band', required=False, 
              help='If you are comparing the first and third vibronic band for\
              a dye, (i.e. pyrene), enter it here. Otherwise, just press enter')
def cac_graphing(path, wv_range, min_conc, max_conc, step, vb1, vb3=None):
    '''
